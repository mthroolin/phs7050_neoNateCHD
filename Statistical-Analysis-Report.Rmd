...
We then performed a check of the linearity assumption by plotting residuals against their fitted values. 
Lastly a posterior predictive distribution check was made by making nine replicates of the data and 
...
The results of both models, one including pre-op attention scores and the other including post-op attention scores,
were exponentiated to be on the odds ratio scale and are shown in @tbl-zoib_summmary_prekable and
@tbl-zoib_summmary_postkable respectively. For the dichotomous attention score models, there was evidence of a
relationship between the the probability of having full oral feeds at discharge and pre-operation attention scores
(see @tbl-zoibcatpre), yet there was no evidence of the association existing with post-operation scores (@tbl-zoibcatpost).

To check the assumption of linearity we plotted the residuals versus predicted outcome, which showed strong linearity (see @sec-zoiblinear). 
For a posterior predictive check, nine replicated samples were drawn from the posterior distribution and 
compared with the original data on percent oral feed at discharge (Figures @fig-ppcpre and @fig-ppcpost, @fig-zoibcatpreppc, @fig-zoibcatpostppc). 
The histograms of the replicated samples do not reflect the clear zero-inflation of the original data. 
Further analysis in a thousand draws of the posterior distribution showed that the maximum or minimum (zero and one) were never drawn. 
A similar check conditioning on the outcome being between zero and one also yielded little confidence in conditional model fit. 
The poor model fit to our data suggests our results to be highly suspect. Future work should address these concerns, 
potentially by changing the covariates in the original model- such as adding a design matrix for modeling the log variance- or selecting another model entirely.
